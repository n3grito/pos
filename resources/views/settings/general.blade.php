    ...
    <x-content-wrapper>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-4">{{ __('Autenticación de Dos Factores (2FA)') }}</h3>
                        <form method="POST" action="{{ route('settings.general.update') }}">
                            @csrf
                            <div class="space-y-6">
                                <div>
                                    <label class="inline-flex items-center gap-3">
                                        ...
                                        <input type="checkbox" name="2fa_enabled" value="1" class="rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ $twoFactorEnabled ? 'checked' : '' }}>
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Habilitar verificación en dos pasos') }}</span>
                                    </label>
                                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400 ml-7">{{ __('Los usuarios con 2FA activado deberán verificar su identidad con un código enviado a su correo electrónico después de iniciar sesión.') }}</p>
                                </div>
                            </div>
