<x-app-layout>
    <x-slot name="header">{{ __('Editar Lista de Precio') }}: {{ $priceList->name }}</x-slot>
    <x-content-wrapper>
        <form method="POST" action="{{ route('price-lists.update', $priceList) }}">
            @csrf @method('PUT')
            <div class="mb-4">
                <x-input-label for="name" :value="__('Nombre')" />
                <x-text-input id="name" name="name" class="w-full mt-1" value="{{ old('name', $priceList->name) }}" required />
                <x-input-error :messages="$errors->get('name')" class="mt-1" />
            </div>
            <div class="mb-4">
                <label class="inline-flex items-center gap-2 text-sm">
                    <input type="checkbox" name="is_default" value="1" {{ $priceList->is_default ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-600">
                ...
                <div class="max-h-96 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            @php $pivot = $priceList->products->find($product->id); @endphp
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-4 py-2">
                                    <input type="number" step="0.01" min="0" name="prices[{{ $product->id }}]" value="{{ old('prices.' . $product->id, $pivot?->pivot?->price) }}" placeholder="{{ $product->selling_price }}" class="w-28 text-right text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
