<x-app-layout>
    ...
    <x-content-wrapper>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-white dark:bg-gray-800 border border-yellow-200 dark:border-yellow-800 rounded-xl">
                <div class="px-4 py-3 bg-yellow-50 dark:bg-yellow-900/30 border-b border-yellow-200 dark:border-yellow-800 rounded-t-xl flex items-center justify-between">
                    <h3 class="text-sm font-semibold text-yellow-700 dark:text-yellow-400">{{ __('Pendientes') }}</h3>
                    <span class="text-xs font-medium text-yellow-600 dark:text-yellow-400 bg-yellow-100 dark:bg-yellow-900/50 px-2 py-0.5 rounded-full">{{ $pending->count() }}</span>
                </div>
                <div class="p-3 space-y-2 min-h-[200px]">
                    @forelse ($pending as $purchase)
                        <a href="{{ route('purchases.show', $purchase) }}" class="block p-3 bg-white dark:bg-gray-700/50 border border-gray-100 dark:border-gray-600 rounded-lg hover:shadow-md transition-shadow">
                            <div class="flex items-center justify-between mb-1">
                                <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">{{ $purchase->invoice_number }}</span>
                                <span class="text-xs text-gray-500 dark:text-gray-400">{{ $purchase->created_at->format('d/m') }}</span>
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">{{ $purchase->supplier->name ?? '-' }} · {{ currency($purchase->total) }}</div>
                        </a>
                    @empty
                        <p class="text-sm text-gray-400 dark:text-gray-500 text-center py-8">{{ __('No hay compras pendientes') }}</p>
                    @endforelse
                </div>
            </div>
            <div class="bg-white dark:bg-gray-800 border border-green-200 dark:border-green-800 rounded-xl">
                ...
                <div class="p-3 space-y-2 min-h-[200px]">
                    ...
                    @empty
                        <p class="text-sm text-gray-400 dark:text-gray-500 text-center py-8">{{ __('No hay compras completadas') }}</p>
                    @endforelse
                </div>
            </div>
            <div class="bg-white dark:bg-gray-800 border border-red-200 dark:border-red-800 rounded-xl">
                ...
                <div class="p-3 space-y-2 min-h-[200px]">
                    @forelse ($cancelled as $purchase)
                        <a href="{{ route('purchases.show', $purchase) }}" class="block p-3 bg-white dark:bg-gray-700/50 border border-gray-100 dark:border-gray-600 rounded-lg hover:shadow-md transition-shadow opacity-75">
                            <div class="flex items-center justify-between mb-1">
                                <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">{{ $purchase->invoice_number }}</span>
                                <span class="text-xs text-gray-500 dark:text-gray-400">{{ $purchase->created_at->format('d/m') }}</span>
                            </div>
                    ...
                    @empty
                        <p class="text-sm text-gray-400 dark:text-gray-500 text-center py-8">{{ __('No hay compras canceladas') }}</p>
                    @endforelse
                </div>
            </div>
        </div>
