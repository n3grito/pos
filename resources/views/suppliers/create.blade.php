<x-app-layout>
    <x-slot name="header">{{ __('Nuevo Proveedor') }}</x-slot>
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('suppliers.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="lg:col-span-2 space-y-6">
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                    @foreach (\App\Models\Supplier::TYPES as $value => $label)
                                        ...
                                        <label class="relative flex items-center p-3 rounded-lg border-2 cursor-pointer transition-all duration-150" style="border-color: {{ $color }}30; background-color: {{ $color }}08" data-type-option data-border-color="{{ $color }}">
                                            <input type="radio" name="client_type" value="{{ $value }}" {{ old('client_type', 'empresa') === $value ? 'checked' : '' }} class="sr-only">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="tax_id" class="block mt-1 w-full" type="text" name="tax_id" :value="old('tax_id')" required placeholder="{{ __('Número de Identificación Tributaria') }}" />
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-end mt-8 gap-2 sm:space-x-3 sm:gap-0 pt-6 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <x-primary-button class="w-full sm:w-auto justify-center">{{ __('Guardar Proveedor') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
