<x-app-layout>
    <x-slot name="header">
    {{ __('Editar Moneda') }}: {{ $currency->name }}
</x-slot>
                    ...
                    <form method="POST" action="{{ route('currencies.update', $currency) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $currency->name)" required autofocus />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="code" :value="__('Código')" />
                                <x-text-input id="code" class="block mt-1 w-full" type="text" name="code" :value="old('code', $currency->code)" required maxlength="10" />
                                <x-input-error :messages="$errors->get('code')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="symbol" :value="__('Símbolo')" />
                                <x-text-input id="symbol" class="block mt-1 w-full" type="text" name="symbol" :value="old('symbol', $currency->symbol)" required maxlength="10" />
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ __('Valor relativo a la moneda base') }}</p>
                                <x-input-error :messages="$errors->get('exchange_rate')" class="mt-2" />
                            </div>

                            <div>
                                <label class="inline-flex items-center mt-4">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $currency->is_active) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-600 text-blue-600 shadow-sm focus:ring-blue-500">
