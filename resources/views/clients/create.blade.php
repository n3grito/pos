<x-app-layout>
    <x-slot name="header">{{ __('Nuevo Cliente') }}</x-slot>
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('clients.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div class="lg:col-span-1">
                            ...
                            <div class="flex flex-col items-center gap-3">
                                <div id="photoPreview" class="w-36 h-36 rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-700 border-2 border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center">
                                    <svg class="w-10 h-10 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                    </svg>
                                </div>
                                <label class="w-full cursor-pointer">
                                    <span class="block w-full text-center px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">{{ __('Seleccionar foto') }}</span>
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            <div>
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                    @foreach (\App\Models\Client::TYPES as $value => $label)
                                        ...
                                        <label class="relative flex items-center p-3 rounded-lg border-2 cursor-pointer transition-all duration-150 has-[:checked]:border-transparent" style="border-color: {{ $color }}30; background-color: {{ $color }}08" x-data x-init="
                                            $el.classList.toggle('ring-2', false);
                                            $el.querySelector('input').addEventListener('change', function() {
                                                document.querySelectorAll('[data-type-option]').forEach(e => {
                                                    e.classList.remove('ring-2', 'shadow-md');
                                                    e.style.borderColor = e.dataset.borderColor + '30';
                                                });
                                                if (this.checked) {
                                                    $el.classList.add('ring-2', 'shadow-md');
                                        ...
                                        " data-type-option data-border-color="{{ $color }}">
                                            <input type="radio" name="client_type" value="{{ $value }}" {{ old('client_type', 'persona_natural') === $value ? 'checked' : '' }} class="sr-only">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="document_number" class="block mt-1 w-full" type="text" name="document_number" :value="old('document_number')" required placeholder="{{ __('Número de Identificación Tributaria') }}" />
                            ...
                            <div>
                                ...
                                <textarea id="notes" name="notes" rows="2" class="block mt-1 w-full border-gray-300 dark:border-gray-600 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm dark:bg-gray-700 dark:text-gray-300">{{ old('notes') }}</textarea>
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-end mt-8 gap-2 sm:space-x-3 sm:gap-0 pt-6 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <x-primary-button class="w-full sm:w-auto justify-center">{{ __('Guardar Cliente') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
