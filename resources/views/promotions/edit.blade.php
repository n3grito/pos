<x-app-layout>
    <x-slot name="header">
    {{ __('Editar Promoción') }}: {{ $promotion->name }}
</x-slot>
                    ...
                    <form method="POST" action="{{ route('promotions.update', $promotion) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $promotion->name)" required autofocus />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>

                            <div class="md:col-span-2">
                                <x-input-label for="description" :value="__('Descripción')" />
                                <textarea id="description" name="description" rows="3" class="block mt-1 w-full border-gray-300 dark:border-gray-600 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm dark:bg-gray-700 dark:text-gray-300">{{ old('description', $promotion->description) }}</textarea>
                            ...
                            <div>
                                ...
                                <select id="type" name="type" class="block mt-1 w-full border-gray-300 dark:border-gray-600 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm dark:bg-gray-700 dark:text-gray-300">
                                    <option value="percentage" {{ old('type', $promotion->type) === 'percentage' ? 'selected' : '' }}>{{ __('Porcentaje') }}</option>
                                    <option value="fixed" {{ old('type', $promotion->type) === 'fixed' ? 'selected' : '' }}>{{ __('Monto Fijo') }}</option>
                                    <option value="bogo" {{ old('type', $promotion->type) === 'bogo' ? 'selected' : '' }}>{{ __('BOGO') }}</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="value" class="block mt-1 w-full" type="number" step="0.01" min="0" name="value" :value="old('value', $promotion->value)" required />
                                <x-input-error :messages="$errors->get('value')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="min_amount" :value="__('Monto Mínimo')" />
                                <x-text-input id="min_amount" class="block mt-1 w-full" type="number" step="0.01" min="0" name="min_amount" :value="old('min_amount', $promotion->min_amount)" />
                                <x-input-error :messages="$errors->get('min_amount')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="min_quantity" :value="__('Cantidad Mínima')" />
                                <x-text-input id="min_quantity" class="block mt-1 w-full" type="number" min="0" name="min_quantity" :value="old('min_quantity', $promotion->min_quantity)" />
                                <x-input-error :messages="$errors->get('min_quantity')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="max_discount" :value="__('Descuento Máximo')" />
                                <x-text-input id="max_discount" class="block mt-1 w-full" type="number" step="0.01" min="0" name="max_discount" :value="old('max_discount', $promotion->max_discount)" />
                            ...
                            <div>
                                ...
                                <select id="applies_to" name="applies_to" class="block mt-1 w-full border-gray-300 dark:border-gray-600 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm dark:bg-gray-700 dark:text-gray-300">
                                    <option value="all" {{ old('applies_to', $promotion->applies_to) === 'all' ? 'selected' : '' }}>{{ __('Todos los productos') }}</option>
                                    <option value="products" {{ old('applies_to', $promotion->applies_to) === 'products' ? 'selected' : '' }}>{{ __('Productos específicos') }}</option>
                                    <option value="groups" {{ old('applies_to', $promotion->applies_to) === 'groups' ? 'selected' : '' }}>{{ __('Grupos de clientes') }}</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="start_date" class="block mt-1 w-full" type="date" name="start_date" :value="old('start_date', $promotion->start_date->format('Y-m-d'))" required />
                                <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="end_date" :value="__('Fecha de Fin')" />
                                <x-text-input id="end_date" class="block mt-1 w-full" type="date" name="end_date" :value="old('end_date', $promotion->end_date->format('Y-m-d'))" required />
                                <x-input-error :messages="$errors->get('end_date')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="usage_limit" :value="__('Límite de Usos')" />
                                <x-text-input id="usage_limit" class="block mt-1 w-full" type="number" min="0" name="usage_limit" :value="old('usage_limit', $promotion->usage_limit)" />
                            ...
                            <div class="flex items-center mt-6">
                                <label class="flex items-center">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $promotion->is_active) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-gray-700">
                                    <span class="ml-2 text-sm text-gray-600 dark:text-gray-400">{{ __('Activa') }}</span>
                                </label>
                            </div>
                        </div>

                        <div id="products-section" class="mt-6 {{ old('applies_to', $promotion->applies_to) === 'products' ? '' : 'hidden' }}">
                        ...
                        <div id="groups-section" class="mt-6 {{ old('applies_to', $promotion->applies_to) === 'groups' ? '' : 'hidden' }}">
                                ...
                                @foreach ($groups as $group)
                                    <label class="flex items-center space-x-2 text-sm">
                                        <input type="checkbox" name="group_ids[]" value="{{ $group->id }}"
                                            {{ in_array($group->id, old('group_ids', $promotion->customerGroups->pluck('id')->toArray())) ? 'checked' : '' }}
                                            class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-gray-700">
