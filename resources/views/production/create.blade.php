                    ...
                    <form method="POST" action="{{ route('production.store') }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="quantity" class="block mt-1 w-full" type="number" step="0.001" min="0.001" name="quantity" :value="old('quantity', '1')" required />
                                <x-input-error :messages="$errors->get('quantity')" class="mt-2" />
                            </div>

                            <div class="md:col-span-2">
                                <x-input-label for="notes" :value="__('Notas')" />
                                <textarea id="notes" name="notes" rows="2" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('notes') }}</textarea>
                                ...
                                <div class="flex items-center space-x-2 material-row">
                                    <select name="items[0][product_id]" required class="flex-1 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
                                        ...
                                        @foreach ($rawMaterials as $mat)
                                            <option value="{{ $mat->id }}" {{ old('items.0.product_id') == $mat->id ? 'selected' : '' }}>
                                                {{ $mat->name }} ({{ $mat->sku }}) — Stock: {{ $mat->stock }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="number" step="0.001" name="items[0][quantity]" min="0.001" value="{{ old('items.0.quantity', '1') }}" required placeholder="{{ __('Cant.') }}"
                        ...
                        <div class="flex items-center justify-end mt-8 space-x-3">
                            ...
                            <x-primary-button>{{ __('Guardar Orden') }}</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
